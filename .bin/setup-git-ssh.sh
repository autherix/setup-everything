...
if dpkg --get-selections | grep -q '^ssh[[:space:]]*install$'; then
...
while [ $ssh_pvt_key_done -eq 0 ]; do
    ...
    if [ -f "$private_key" ]; then
        ...
        if [[ $(head -n 1 "$private_key") == "-----BEGIN OPENSSH PRIVATE KEY-----" ]] && [[ $(tail -n 1 "$private_key") == "-----END OPENSSH PRIVATE KEY-----" ]]; then
...
echo
if ! grep -q "eval \$(ssh-agent -s)" /ptv/add_to_bashrc.sh; then
...
# fi
if ! grep -q "gitssh()" /ptv/add_to_bashrc.sh; then
