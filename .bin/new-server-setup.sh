...
if grep -Fxq "ssh" /tmp/installed-packages.txt; then
...
if [[ $configure_ssh =~ ^[Yy]$ ]]; then
    ...
    while [ $port_set -ne 1 ]; do
        ...
        elif ! [[ "$ssh_port" =~ ^[0-9]+$ ]] || [ "$ssh_port" -lt 1025 ] || [ "$ssh_port" -gt 65535 ]; then
            echo "[-] Error: Invalid port number, please enter a valid port number"
        # Check if the port is already in use
        elif [ $(sudo lsof -i -P -n | grep LISTEN | grep -c "$ssh_port") -eq 0 ]; then
...
if grep -Fxq "docker" /tmp/installed-packages.txt; then
...
if grep -Fxq "docker-compose" /tmp/installed-packages.txt; then
...
if grep -Fxq "ocserv" /tmp/installed-packages.txt; then
...
if [ -z "$configure_ocserv" ]; then
    configure_ocserv="n"
fi
# If configure_ocserv is "y" or "Y", configure ocserv
if [[ $configure_ocserv =~ ^[Yy]$ ]]; then
    # Install ocserv
...
if grep -Fxq "openvpn" /tmp/installed-packages.txt; then
...
if grep -Fxq "go" /tmp/installed-packages.txt; then
...
if [ -z "$configure_go" ]; then
    configure_go="n"
fi
# If configure_go is "y" or "Y", configure go
if [[ $configure_go =~ ^[Yy]$ ]]; then
    # Install go
    echo "[+] Installing go..."
    ...
    if ! grep -q "export PATH=\$PATH:/usr/local/go/bin" /ptv/add_to_path.sh; then
    ...
    if ! grep -q "export PATH=\$PATH:/ptv/go/bin" /ptv/add_to_path.sh; then
        echo "export PATH=\$PATH:/ptv/go/bin" >>/ptv/add_to_path.sh
    fi
    # export GOROOT=/usr/local/go
    if ! grep -q "export GOROOT=/usr/local/go" /ptv/add_to_path.sh; then
    ...
    if ! grep -q "export GOPATH=/ptv/go" /ptv/add_to_path.sh; then
    ...
    if ! grep -q "export GOBIN=/ptv/go/bin" /ptv/add_to_path.sh; then
...
if grep -Fxq "tools" /tmp/installed-packages.txt; then
...
if [ -z "$configure_tools" ]; then
    configure_tools="n"
fi
# If configure_tools is "y" or "Y", configure tools
if [[ $configure_tools =~ ^[Yy]$ ]]; then
    # Install tools
...
if grep -Fxq "bashrc, aliases, path" /tmp/installed-packages.txt; then
...
if [[ $configure_bashrc_aliases_path =~ ^[Yy]$ ]]; then
    ...
    if [ ! -s /ptv/add_to_bashrc.sh ]; then
    ...
    if ! grep -q "source /ptv/add_to_bashrc.sh" ~/.bashrc; then
    ...
    if [ ! -s /ptv/add_to_aliases.sh ]; then
    ...
    if ! grep -q "source /ptv/add_to_aliases.sh" /ptv/add_to_bashrc.sh; then
    ...
    if ! grep -q "source /ptv/add_to_path.sh" /ptv/add_to_bashrc.sh; then
    ...
    for alias in "${aliases[@]}"; do
        ...
        if ! grep -q "$checking_alias" /ptv/add_to_aliases.sh; then
    ...
    if ! grep -q "source /ptv/rtsetup.sh" /ptv/add_to_bashrc.sh; then
    ...
    if ! grep -q "rt=/ptv" /ptv/rtsetup.sh; then
    ...
    if ! grep -q "alias rt='cd /ptv'" /ptv/rtsetup.sh; then
    ...
    if ! grep -q "alias rtsetup='source \$rt/rtsetup.sh'" /ptv/rtsetup.sh; then
    ...
    if ! grep -q "function seterror() { errmsg=\$1; echo -e \"\\e[31m[ERR] \$errmsg\\e[0m\"; exit 1; }" /ptv/rtsetup.sh; then
    ...
    if ! grep -q "bashrc, aliases, path" /tmp/installed-packages.txt; then
